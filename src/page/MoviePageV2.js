...
function MoviePageV2() {
  ...
  const { data, error, size, setSize } = useSWRInfinite(
    (index) =>
      `https://api.themoviedb.org/3${filterDebounce ? "/search" : ""}/movie${
        !filterDebounce ? "/popular" : ""
      }?api_key=${api_key}&language=en-US&page=${index + 1}${
        filterDebounce ? `&query=${filterDebounce}` : ""
      }`,
    fetcher
  );
  ...
  const movies = data
    ? data.reduce(
        (initialValue, item) => [...initialValue, ...item?.results],
        []
      )
    : [];
  ...
  return (
    <section className="container-base">
      <div className="flex items-center mx-auto w-full max-w-[600px] mb-5">
        ...
        <button className="ml-5 py-3 px-5 rounded-md bg-primary border-none outline-none text-base text-white font-semibold">
          <svg
      ...
      {isLoading ? (
        ...
        <div
          className="page-container grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3
         lg:grid-cols-4 gap-3 md:gap-6"
        >
          {movies.length > 0 &&
            movies.map((movie) => {
              return (
      ...
      <div className="py-10 text-center">
        <button
          ...
          className="outline-none border-none rounded-md bg-primary text-white 
          text-sm p-3 hover:brightness-90 disabled:bg-gray-400"
        >
          <span>Load More</span>
        </button>
      </div>
